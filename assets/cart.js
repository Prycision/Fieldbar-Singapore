  ...
  updateQuantity(line, quantity, name, variantId) {
    ...
    fetch(`${routes.cart_change_url}`, { ...fetchConfig(), ...{ body } })
      ...
      .then((state) => {
        ...
        const itemCount = Number(parsedState.item_count) || 0;

        if (cartFooter) {
          const isEmpty = parsedState.item_count === 0;
          const wasEmpty = cartFooter?.classList.contains("is-empty");
          cartFooter?.classList.toggle("is-empty", itemCount === 0);
          if (wasEmpty === false && itemCount === 0) location.reload();
        }
...
document.addEventListener("click", function (event) {
  const button = event.target.closest("#CartDrawer-Checkout");
  if (button) {
    event.preventDefault();
